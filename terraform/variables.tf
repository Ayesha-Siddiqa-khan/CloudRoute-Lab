...
variable "gateway_http_node_port" {
  description = "Fixed HTTP NodePort used by the Gateway API data plane and the external NGINX load balancer"
  type        = number
  default     = 30080
}

variable "gateway_nodeport_allowed_cidr" {
  description = "CIDR allowed to reach the Gateway API HTTP NodePort. Restrict this to the external NGINX EC2 private/public IP range when possible."
  type        = string
  default     = "0.0.0.0/0"
}
...
variable "ecr_repository_name_1" {
  ...
  type        = string
  default     = "cloudroute-lab-backend"
}
...
variable "ecr_repository_name_2" {
  ...
  type        = string
  default     = "cloudroute-lab-frontend"
}
...
variable "app_hostname" {
  ...
  type        = string
  default     = "cloudroute-lab.example.com"
}
