...
output "user_data_cloudwatch_log_group" {
  description = "CloudWatch Logs group for EC2 user-data logs"
...
output "kubernetes_container_cloudwatch_log_group" {
  description = "CloudWatch Logs group used by the Kubernetes Fluent Bit DaemonSet"
  value       = aws_cloudwatch_log_group.kubernetes_container_logs.name
}

output "gateway_http_node_port" {
  description = "Fixed HTTP NodePort expected by the external NGINX EC2 load balancer"
...
output "kubestate_recovery_lab_next_steps" {
  ...
  value = [
    ...
    "Install Gateway API CRDs and Envoy Gateway, then apply k8s/gateway-api/*.yaml.",
