...
resource "aws_iam_role_policy_attachment" "worker_ec2_user_data_cloudwatch_logs" {
  role       = aws_iam_role.worker_ec2_ecr_pull.name
...
locals {
  role_trust_policies = {
    ...
    "cloudroute-lab-dev-role" = jsonencode({
      Version = "2012-10-17"
      Statement = [{
        Effect = "Allow"
        Principal = {
...
resource "aws_iam_role" "roles" {
  ...
  assume_role_policy = local.role_trust_policies[each.key]
  description        = "IAM role for ${each.value.type}"
...
resource "aws_iam_role_policy_attachment" "role_policy_attachments" {
  for_each = {
    for attachment in local.role_policy_attachments : "${attachment.role_name}-${attachment.policy_arn}" => attachment
  }
