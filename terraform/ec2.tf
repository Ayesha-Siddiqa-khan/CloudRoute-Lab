...
resource "aws_s3_object" "bootstrap_kubernetes_master_user_data" {
  ...
  content = templatefile("${path.module}/scripts/master-user-data.sh", {
    ...
    ssm_kubeconfig_public_b64_path = local.terrapilot_ssm_kubeconfig_public_b64_path
...
locals {
  ...
  terrapilot_ssm_kubeconfig_public_b64_path = "/terrapilot/${local.project_slug}/${local.environment_slug}/kubernetes/kubeconfig/public-b64"
...
resource "aws_cloudwatch_log_group" "terrapilot_user_data" {
  ...
  tags = {
    Name                   = "${local.resource_prefix}-user-data-logs"
    Project                = var.project_name
    TerraPilotProject      = var.project_name
    TerraPilotResourceType = "cloudwatch-log-group"
    Environment            = var.environment
    ManagedBy              = "TerraPilot"
    CostSensitive          = "true"
  }
...
resource "aws_iam_policy" "terrapilot_user_data_cloudwatch_logs" {
  name        = "${local.resource_prefix}-user-data-logs"
  description = "Allow EC2 user-data scripts to stream cloud-init and TerraPilot logs to CloudWatch Logs."
...
resource "aws_cloudwatch_log_group" "kubernetes_container_logs" {
  ...
  tags = {
    Name                   = "${local.resource_prefix}-kubernetes-container-logs"
    Project                = var.project_name
    TerraPilotProject      = var.project_name
    TerraPilotResourceType = "cloudwatch-log-group"
    Environment            = var.environment
...
resource "aws_iam_policy" "worker_ec2_cloudwatch_container_logs" {
  name        = "${local.resource_prefix}-worker-container-logs"
  description = "Allow Kubernetes Fluent Bit pods running on worker nodes to send container logs to CloudWatch Logs using the EC2 instance role."
...
resource "aws_iam_role_policy_attachment" "worker_ec2_cloudwatch_container_logs" {
  role       = aws_iam_role.worker_ec2_ecr_pull.name
  policy_arn = aws_iam_policy.worker_ec2_cloudwatch_container_logs.arn
}

resource "aws_iam_role_policy_attachment" "terrapilot_user_data_cloudwatch_logs" {
  role       = aws_iam_role.terrapilot_ec2_userdata.name
  policy_arn = aws_iam_policy.terrapilot_user_data_cloudwatch_logs.arn
}
...
resource "aws_instance" "main" {
  ...
  depends_on = [
    ...
    aws_iam_role_policy_attachment.terrapilot_user_data_cloudwatch_logs,
    ...
    aws_iam_role_policy_attachment.worker_ec2_user_data_cloudwatch_logs,
    aws_iam_role_policy_attachment.worker_ec2_cloudwatch_container_logs,
